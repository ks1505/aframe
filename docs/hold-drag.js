...
AFRAME.registerComponent('hold-drag', {
  ...
  init: function() {
    ...
    tapRingInner.setAttribute('color', '#ffffff')
    tapRingInner.setAttribute('ring-inner', '0')
    tapRingInner.setAttribute('ring-outer', '0')
    tapRingInner.setAttribute('rotation', '-90 0 0')
    tapRingInner.setAttribute('position', '0 -2 0')
  ...
  fingerUp: function(event) {
    ...
    if (this.internalState.dragging) {
      ...
      this.el.setAttribute('animation__drop', {
        property: 'position',
        to: `${endPosition.x} -2 ${endPosition.z}`,
        dur: 300,
